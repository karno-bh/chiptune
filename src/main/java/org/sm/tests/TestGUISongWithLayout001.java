...
public class TestGUISongWithLayout001 {
    ...
    public TestGUISongWithLayout001() {
        loadIcons();
        initSongProcessor();
        initGUI();
    }
...
//        directoryPath.setColumns(20);
    ...
    private void loadSong(String songLocation) {
        song = new FYMSong2(songLocation);
    ...
    private void loadSelectedDirectory(File directory) {
        ...
        for (File f : files) {
            ...
            row.add(f.getName());
            try {
                Song s = new FYMSong2(f.getAbsolutePath(), true);
                row.add(s.getAuthor());
                row.add(s.getTrack());
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(TestGUISongWithLayout001::new);
    }
